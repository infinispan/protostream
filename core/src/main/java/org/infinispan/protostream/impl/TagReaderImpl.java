...
import org.infinispan.protostream.ByteArrayBacked;
...
public final class TagReaderImpl implements TagReader, ProtobufTagMarshaller.ReadContext {
   ...
   public static TagReaderImpl newInstance(ImmutableSerializationContext serCtx, InputStream input, int length) throws IOException {
      Decoder decoder;
      if (input instanceof ByteArrayBacked bab) {
         if (length != bab.length()) {
            throw new IllegalArgumentException("Provided length " + length + " doesn't match the ByteArrayBacked size: " + bab.length());
         }
         decoder = new ByteArrayDecoder(bab.bytes(), bab.offset(), length);
      } else if (length < 128) {
