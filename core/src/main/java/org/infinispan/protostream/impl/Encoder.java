...
abstract class Encoder {
   ...
   static Encoder newInstance(byte[] array, int offset, int length) {
      return new ByteArrayEncoder(array, offset, length);
   }

   static Encoder newInstance(ByteBuffer buffer) {
      return buffer.hasArray() ? new HeapByteBufferEncoder(buffer) : new ByteBufferEncoder(buffer);
   }

   static Encoder newInstance(OutputStream output, int bufferSize) {
   ...
   public final void writeInt32(int number, int value) throws IOException {
      if (value >= 0) {
         writeUInt32Field(number, value);
      } else {
         writeUInt64Field(number, value);
      }
   }

   public final void writeSInt32(int number, int value) throws IOException {
      // Roll the bits in order to move the sign bit from position 31 to position 0, to reduce the wire length of negative numbers.
      writeUInt32Field(number, (value << 1) ^ (value >> 31));
   }
   ...
   public final void writeDouble(int number, double value) throws IOException {
      writeFixed64Field(number, Double.doubleToRawLongBits(value));
   }

   public final void writeFloat(int number, float value) throws IOException {
      writeFixed32Field(number, Float.floatToRawIntBits(value));
   }
   ...
   public abstract void writeVarint32(int value) throws IOException;

   public abstract void writeVarint64(long value) throws IOException;
   ...
   public abstract void writeBytes(ByteBuffer value) throws IOException;

   /**
    * An encoder that just counts the bytes and does not write anything. Useful for computing message size.
    */
   ...
   private static final class ByteBufferEncoder extends Encoder {
      ...
      /**
       * If JVM is not LITTLE_ENDIAN as expected by Protobuf binary format then we need to reverse bytes
   ...
   private static final class OutputStreamEncoder extends Encoder {
      ...
      OutputStreamEncoder(OutputStream out, int bufferSize) {
         // Must fit at least 2 varints without having to flush, so we can write the biggest possible tag and also the
         // biggest possible field value, except for length delimited fields which can be arbitrarily big, but at least
         // their length varint should fit.
